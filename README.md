...
Due to file size constraints, the complete ERP dataset cannot be provided in full. I will send you an ERP.zip file containing only the data from the first participant for reference purposes. Please note that this data is intended solely for your review and should not be used for any other purposes.
To reproduce our experiments, you may use the training and validation splits ("Train" and "VAL"These two datasets contain the corresponding textual data, as well as all labels for the three-category metaphor novelty classification and complete ERP information.) via Train1/2.py. You are also free to replace the original model in the script with other models for training.
